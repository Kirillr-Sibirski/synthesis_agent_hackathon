...
const CONFIG_PATH = process.env.FRONTEND_CONFIG_PATH ?? 'apps/web/public/config.json';
...
function roleWarnings(actors: FrontendConfig['actors']) {
  ...
  return {
    ...
    allowedDuplicates,
    blockingDuplicates,
    readySeparation: blockingDuplicates.length === 0,
  };
}
...
function main() {
  ...
  if (!existsSync(resolvedConfig)) {
    throw new Error(`Frontend config not found at ${resolvedConfig}. Generate it first with bun run frontend:write-config.`);
  }
  ...
  const preflightChecks = preflight
    ? {
        ...
        expectedExecutor:
          asAddress(preflight.accounts?.treasuryExecutor) ??
          asAddress(preflight.accounts?.delegatorSmartAccount) ??
          asAddress(preflight.accounts?.executor),
        treasuryMatches:
  ...
  const report = {
    ...
    readiness: {
      readyForFrontendSameNetworkDemoConfig:
        missing.length === 0 &&
        warnings.readySeparation &&
        (!preflight || preflight.readiness?.readyForFinalSameNetworkRun === true || preflightChecks.loaded === false),
      missing,
      warnings: [
        ...(warnings.blockingDuplicates.length
          ? [`Role overlap still present in frontend config: ${JSON.stringify(warnings.blockingDuplicates)}`]
          : []),
