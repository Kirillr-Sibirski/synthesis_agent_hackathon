...
const CHECK_TO_ENV: Record<string, string[]> = {
  ...
  wstETHConfigured: ['WSTETH_ADDRESS=0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452'],
  wstETHIsBaseMainnet: ['WSTETH_ADDRESS=0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452'],
  ...
  frontendAssetConfigured: ['FRONTEND_ASSET_ADDRESS=0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452'],
  frontendAssetIsBaseMainnet: ['FRONTEND_ASSET_ADDRESS=0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452'],
...
function main() {
  ...
  if (!existsSync(resolvedValidation)) {
    throw new Error(
      `Cutover env validation artifact not found at ${resolvedValidation}. Run bun run final:validate-cutover-env first.`,
    );
  }
...
- expected canonical mainnet \`wstETH\`: \`${expectedChain.expectedWstETH ?? '0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452'}\`
...
- Re-run:
  - \`CUTOVER_ENV_VALIDATION_OUT=artifacts/final/cutover-env-validation.json bun run final:validate-cutover-env\`
