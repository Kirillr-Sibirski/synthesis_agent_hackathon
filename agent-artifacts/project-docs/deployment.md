...
- see `agent-artifacts/deployments/base-mainnet-metamask-live.md` for the current addresses and tx hashes
...
Use `agent-artifacts/deployments/base-mainnet-cutover-template.md` as the single place to record the final same-network run:
...
- frontend cutover values for `frontend/public/config.json`
...
That writes the local `frontend/public/config.json` from deployment values instead of relying on hand-edited JSON under deadline pressure.
