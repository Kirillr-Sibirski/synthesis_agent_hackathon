...
- `ReceiptRegistry`:
- `wstETH` token address: `0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452`
...
### Dashboard config
Populate `apps/web/public/config.json` locally with:
- chain: `base`
- treasury:
- asset:
- receiptRegistry:
...
METAMASK_PREFLIGHT_PATH=artifacts/metamask/preflight-8453.json \
  bun run frontend:validate-config
```
...
FRONTEND_VALIDATION_PATH=artifacts/frontend/validation.json \
  bun run final:validate-same-network
```
...
CUTOVER_ENV_VALIDATION_OUT=artifacts/final/cutover-env-validation.json \
  bun run final:validate-cutover-env
```
...
That bundle now also writes:
- `artifacts/final/cutover-env-checklist.md` — a generated env/operator handoff derived from the cutover validator, including exact missing env inputs and signer/address consistency checks
- `artifacts/final/cutover-checklist.md` — a human-readable handoff/checklist summarizing honest track posture, current blockers, and the exact remaining proof gaps before the final Base mainnet demo/submission

Expected env inputs:
- recommended starting point: copy `env/base-mainnet-cutover.example.env` into local `.env` and fill it for the final run
- `bun run final:validate-cutover-env` now also reports whether declared owner/executor/demo/frontend addresses are internally consistent with the loaded private keys and role wiring, so use it before any live mainnet attempt
