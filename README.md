...
- MetaMask live-redemption readiness can now be checked in one step (`npm run metamask:preflight`) for env completeness, treasury code presence, smart-account deployment status, bundler reachability plus bundler/chain alignment, exact spend selector details, and final same-network Base mainnet readiness
...
- that same bundle now also emits a generated Base mainnet cutover-env checklist (`artifacts/final/cutover-env-checklist.md`) so the operator can fill the remaining final-run env inputs without drifting from the validator output
