...
contract WstETHYieldTreasuryTest is Test {
    ...
    function testFallsBackToRawWstETHAccountingForBridgeTokens() external {
        MockERC20 l2Asset = new MockERC20("Bridged Wrapped stETH", "wstETH", 18);

        vm.prank(owner);
        WstETHYieldTreasury l2Treasury = new WstETHYieldTreasury(address(l2Asset), owner);

        vm.prank(owner);
        DelegationAuthorizer l2Authorizer = new DelegationAuthorizer(owner);

        vm.prank(owner);
        ReceiptRegistry l2Receipts = new ReceiptRegistry(address(l2Treasury));

        vm.prank(owner);
        l2Treasury.setAuthorizer(address(l2Authorizer));

        vm.prank(owner);
        l2Treasury.setReceiptRegistry(address(l2Receipts));

        l2Asset.mint(depositor, 20 ether);
        vm.prank(depositor);
        l2Asset.approve(address(l2Treasury), type(uint256).max);

        vm.prank(depositor);
        l2Treasury.deposit(8 ether);

        assertFalse(l2Treasury.rateConversionsAvailable());
        assertEq(l2Treasury.principalBaselineStETH(), 0);
        assertEq(l2Treasury.principalBaselineWstETH(), 8 ether);
        assertEq(l2Treasury.currentPrincipalFloorWstETH(), 8 ether);
        assertEq(l2Treasury.availableYieldInStETH(), 0);
        assertEq(l2Treasury.availableYieldInWstETH(), 0);

        vm.prank(depositor);
        l2Asset.transfer(address(l2Treasury), 4 ether);

        vm.prank(owner);
        l2Treasury.configureBudget(OPS_BUDGET, bytes32(0), manager, 3 ether, true, "ops");

        bytes32 ruleId = _authorizeFor(l2Authorizer, l2Treasury, executor, OPS_BUDGET, recipient, 3 ether);
        bytes32 receiptHash = keccak256("l2-wsteth-receipt-1");

        vm.prank(executor);
        l2Treasury.spendFromBudget(
            OPS_BUDGET,
            recipient,
            2 ether,
            keccak256("l2-wsteth-task-1"),
            receiptHash,
            keccak256("l2-wsteth-evidence-1"),
            keccak256("l2-wsteth-result-1"),
            "ipfs://l2-wsteth-1"
        );

        assertEq(l2Asset.balanceOf(recipient), 2 ether);
        assertEq(l2Asset.balanceOf(address(l2Treasury)), 10 ether);
        assertEq(l2Treasury.currentPrincipalFloorWstETH(), 8 ether);
        assertEq(l2Treasury.availableYieldInWstETH(), 2 ether);

        (, bytes32 storedRuleId,,,,,,,,) = l2Receipts.receipts(receiptHash);
        assertEq(storedRuleId, ruleId);
    }
    ...
    {
        return _authorizeFor(authorizer, treasury, ruleExecutor, budgetId, ruleRecipient, maxAmount);
    }

    function _authorizeFor(
        DelegationAuthorizer targetAuthorizer,
        WstETHYieldTreasury targetTreasury,
        ...
        ruleId = keccak256(
            abi.encode(ruleExecutor, budgetId, ruleRecipient, targetTreasury.spendFromBudget.selector)
        );
        DelegationAuthorizer.Rule memory rule = DelegationAuthorizer.Rule({
            ...
            selector: targetTreasury.spendFromBudget.selector,
            maxAmount: maxAmount,
            validAfter: 0,
            validUntil: 0
        });
        ...
        vm.prank(owner);
        targetAuthorizer.setRule(ruleId, rule);
    }
}
