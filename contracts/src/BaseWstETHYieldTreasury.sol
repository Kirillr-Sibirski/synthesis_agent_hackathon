...
/// @notice Base's bridged wstETH token does not expose L1 conversion helpers, so
/// principal/yield accounting is tracked directly in wstETH units while keeping
/// the same external ABI the rest of the repo expects.
contract BaseWstETHYieldTreasury {
    struct Budget {
        uint128 allocated;
        uint128 spent;
        bool active;
